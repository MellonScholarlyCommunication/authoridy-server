...
async function handle(id,date) {
    const biblioDate = dateToBiblio(date);

    const res = await fetch(`${BIBLIO_BASE_URL}/person/${id}/publication/export?format=json`);
    ...
    try {
        ...
        for (let i = 0 ; i < lines.length ; i++) {
            if (lines[i].length) {
                ...
                else if (date_updated.localeCompare(biblioDate)>=0) {
                    result['contributions'].push(record); 
                }
            }
        }
